# Source: https://github.com/victordibia/handtracking/blob/master/egohands_dataset_clean.py
# This is the code used to generate the csv files needed to train tfrecords for training ssd mobilenet
...
def get_bbox_visualize(base_path, dir):
    ...
    # Sort image_path_array to ensure it's in the low-to-high order expected in polygon.mat
    ...
    for first in polygons:
        ...
        cv2.waitKey(2)  # Closes window when a key press is detected
...
# Combines all individual csv files for each image into a single csv file per folder.
...
# Splits data, copies to train/test folders
...
# Renames image files with unique names so we can have them all in a train/test/eval folder.
...
def download_egohands_dataset(dataset_url, dataset_path):
    ...
    if not is_downloaded:
        print(
            "> Downloading egohands dataset. Requires 1.3GB, ~3-5 mins.")
